...
8. `_run_coroutine(_run_mosaic_all_dates(...))` drives all time steps from a single event loop invocation. `_run_coroutine` delegates to `_run_in_fresh_loop` normally (creates a fresh loop, installs the bounded executor before any coroutines run, then tears both down) or, when called from inside a running event loop (e.g. a Jupyter kernel), submits the coroutine to a persistent per-thread background loop via `asyncio.run_coroutine_threadsafe`. Inside `_run_mosaic_all_dates`, an `asyncio.gather` fans out one `_run_one_date` coroutine per time step, so COG reads overlap across dates:
...
**Bounded per-loop executor.** Rather than using Python's default `min(32, cpu_count + 4)` thread count, `_run_in_fresh_loop()` installs a bounded `ThreadPoolExecutor` (default `min(os.cpu_count(), 4)`) as the default executor on each event loop it creates, before any coroutines run. This caps thread count per loop while preserving per-loop isolation: each dask task has its own independent pool and does not queue behind other tasks. The executor is shut down in `_run_in_fresh_loop`'s `finally` block after the loop closes, so no threads leak. Call `lazycogs.set_reproject_workers(n)` to change the per-loop bound (see `_executor.py`).
