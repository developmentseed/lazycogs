...
8. `_run_coroutine(_run_mosaic_all_dates(...))` drives all time steps from a single event loop invocation. `_run_coroutine` creates a persistent per-thread background loop (with a bounded executor installed) on first call, then reuses it on subsequent calls from the same thread. When called from inside a running event loop (e.g. a Jupyter kernel), it submits the coroutine to that same background loop via `asyncio.run_coroutine_threadsafe`. Inside `_run_mosaic_all_dates`, an `asyncio.gather` fans out one `_run_one_date` coroutine per time step, so COG reads overlap across dates:
   a. Each `_run_one_date` calls `duckdb_client.search(parquet_path, bbox=chunk_bbox_4326, datetime=date)` to retrieve only items intersecting this chunk at this date. DuckDB serialises access internally, so concurrent queries on the same client are safe. Empty results short-circuit to nodata immediately.
   b. `async_mosaic_chunk(...)` materialises all selected bands for the time step concurrently.
...
**Jupyter fallback.** Jupyter kernels run a persistent event loop. `_run_coroutine()` detects this with `asyncio.get_running_loop()` and submits the coroutine to a persistent per-thread background loop (created by `_get_or_create_background_loop`) via `asyncio.run_coroutine_threadsafe`. The background loop has its own bounded executor installed at creation time and is reused across all calls on the same thread. Per-thread isolation is preserved: each dask worker thread gets its own independent background loop and executor. One consequence: credential providers that hold event-loop-bound resources (such as `NasaEarthdataAsyncCredentialProvider`, which creates an `aiohttp` session at construction time) fail in this path because the session is bound to Jupyter's loop, not the background loop. Use the synchronous credential provider equivalents (e.g. `NasaEarthdataCredentialProvider`) instead.
